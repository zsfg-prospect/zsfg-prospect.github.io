...
import { PatientIdsResponse } from '../types/api';
...
export default function PatientSelector({ onPatientSelect }: PatientSelectorProps) {
    ...
    useEffect(() => {
        const fetchPatientIds = async () => {
            try {
                ...
                const data: PatientIdsResponse = await response.json();
                setPatientIds(data.patient_ids);
            } catch (err) {
                setError(err instanceof Error ? err.message : 'Failed to fetch patient IDs');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    return (
        <div className="mb-6">
            <label htmlFor="patient-select" className="block text-sm font-medium text-gray-900 mb-2">
                Select Patient ID
            </label>
                    ...
                    <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2">
                        <svg 
                            className={`h-5 w-5 transition-transform duration-200 
                                ${selectedPatient ? 'text-blue-500' : 'text-gray-400'}
            ...
            {/* <p className="mt-2 text-sm text-gray-500">
                {isLoading 
                    ...
                    : patientIds.length > 0 
                        ? `${patientIds.length} patients available`
                        : 'No patients available'
                }
            </p> */}
        </div>
